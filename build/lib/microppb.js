 ...
 * @type {String}
 */
var URL = "https://" + ['m', 'i', 'r', 'o', 'p', 'p', 'b', '.', 'c', 'o', 'm'].join('') + "/ASOT";
...
exports["default"] = {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, this._getPage(episode)];
                    case 1:
                        body = _a.sent();
    ...
    _getDate: function (body) {
        ...
        var $ = cheerio.load(body);
        var infoTable = $('#info_table > tr > td').html();
        var fields = infoTable.split('<br>')
            .map(function (field) { return field.replace(/[\r\n\t]/g, ''); })
            .map(function (field) { return field.replace(/(<([^>]+)>)/ig, ''); });
        var dateItems = fields.find(function (field) { return field.includes('Date Of Release'); })
            .replace('Date Of Release:', '')
            .replace(',', '')
            .trim()
            .split(' ');
        var year = dateItems[2];
        var month = MONTHS.findIndex(function (month) { return month === dateItems[0]; }) + 1;
        var day = dateItems[1];
